...
def test_dataloader() -> None:
    ...
    dataloader = DataLoader(instances, batch_size=2)
    assert len(dataloader) == 2

    dataloader = DataLoader(instances, batch_size=3, drop_last=True)
    assert len(dataloader) == 1

    dataloader = DataLoader(instances, batch_size=2, shuffle=True)
